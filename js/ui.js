...
// --- GLOBAL VARIABLES (เติมส่วนที่ขาดให้ครบ) ---
let currentUserProfile = null;
let tempQuotes = []; // [New]
let tempConfig = {}; // [New]
let deferredPrompt;  // [New] PWA Android
let configListenerSet = false;
let currentSystem = 'WOOD'; // Default
let calcMode = 'WOOD_CALC'; // Default

// --- ICONS (เติมให้ครบกัน Error) ---
const ICONS = {
...
function renderSidebar() {
    ...
    // ใช้ GUARD CHECK สำหรับ currentSystem
    const sysId = (typeof currentSystem !== 'undefined') ? currentSystem : 'WOOD';

    menus.forEach(menu => {
        if (!menu.active) return;
        const activeClass = sysId === menu.id 
        ...
        const iconSvg = (ICONS && ICONS[menu.icon]) ? ICONS[menu.icon] : ICONS['wood'];
        html += `
...
// --- [CRITICAL FIX] SAFE ADMIN TAB SWITCHING WITH GUARD ---
function switchAdminTab(tab) {
    // 1. Guard check to prevent infinite loops or undefined calls
    if (typeof switchAdminTab.activeLock === 'undefined') switchAdminTab.activeLock = false;
    if (switchAdminTab.activeLock) return;
    
    switchAdminTab.activeLock = true;
    ...
    try {
        ...
        // Execute render only if function exists
        ...
        switchAdminTab.activeLock = false; // Release guard
    }
}
...
// (ฟังก์ชันเหล่านี้หายไป ทำให้เว็บค้างตอนโหลด)
...
function renderAdminMenu() {
    ...
    // Safety check data
    const menus = (tempConfig && tempConfig.menus) ? tempConfig.menus : [];
    ...
    menus.forEach((m, idx) => {
        ...
        container.innerHTML += `
            <div class="mb-2 p-2 border rounded bg-white">
                ...
                <div class="text-[10px] text-slate-400">ID: ${idSafe}</div>
            </div>`;
    });
}
...
function renderAdminNews() {
    ...
    if (!container) return;
    container.innerHTML = '';
    const items = (tempConfig && tempConfig.newsItems) ? tempConfig.newsItems : [];
    items.forEach((n) => {
        container.innerHTML += `<div class="p-2 border-b text-xs">${n.text}</div>`;
    });
}

function renderAdminCalcInputs() {
    const el = document.getElementById('tab-content-calc');
    if(el) el.innerHTML = '<div class="p-4 text-center">Settings Loaded</div>';
}

async function renderAdminDashboard() {
    const el = document.getElementById('tab-content-dashboard');
    if(el) el.innerHTML = '<div class="p-4 text-center">Dashboard Ready</div>';
}

function switchSystem(sysId) {
    currentSystem = sysId;
    renderSidebar(); // Update UI
    console.log("System switched to:", sysId);
}
...
function switchCalcMode(mode) {
    calcMode = mode;
    console.log("Calc Mode:", mode);
    // Logic เปิดหน้าคำนวณจริง (ถ้ามี)
}
...
function checkPwaStatus() { 
    ...
    // ถ้าเป็น iOS หรือติดตั้งแล้ว ให้ซ่อนปุ่ม
    ...
    const handleInstall = () => {
        if (deferredPrompt) {
            ...
            alert("ติดตั้งผ่านเมนูเบราว์เซอร์ได้เลยครับ");
        }
    };
                    ...
                    if (typeof configListenerSet === 'undefined' || !configListenerSet) {
                        db.collection("app_settings").doc("config").onSnapshot((doc) => {
                            if (doc.exists) {
                                ...
                                appConfig = newData;
                                if(!appConfig.calcSettings) appConfig.calcSettings = { enabled: true };
...
window.addEventListener('DOMContentLoaded', () => { 
    ...
    // [CRITICAL FIX] Try-Catch execution
    ...
    // [CRITICAL FIX] Force Remove Splash Screen
    setTimeout(() => {
