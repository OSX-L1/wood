...
// --- ADMIN & UTILS (Keep existing functions) ---
...
// Include Admin Renderers (Shortened for brevity but assumed present)
...
async function renderAdminDashboard() { /* (Keep existing Dashboard logic) */ const container = document.getElementById('tab-content-dashboard'); if (!container) return; container.innerHTML = `<div class="flex flex-col items-center justify-center h-64"><span class="loader w-10 h-10 border-4 border-slate-200 border-t-sunny-red rounded-full mb-4"></span><span class="text-slate-400">กำลังประมวลผลข้อมูล...</span></div>`; try { let quotes = tempQuotes; if (!quotes || quotes.length === 0) { if (db && auth && auth.currentUser) { const snap = await db.collection("quotations").get(); quotes = []; snap.forEach(doc => quotes.push({ ...doc.data(), docId: doc.id })); } else { quotes = JSON.parse(localStorage.getItem('sunny_quotations')) || []; } tempQuotes = quotes; } const totalDocs = quotes.length; let totalValue = 0; let woodCount = 0, pvcCount = 0, rollerCount = 0, aluCount = 0; const last7Days = {}; for(let i=6; i>=0; i--) { const d = new Date(); d.setDate(d.getDate() - i); const k = d.toLocaleDateString('th-TH'); last7Days[k] = 0; } quotes.forEach(q => { const amount = parseFloat((q.total || "0").toString().replace(/,/g, '').replace(/[^0-9.]/g, '')) || 0; totalValue += amount; if (q.type.includes('ไม้')) woodCount++; else if (q.type.includes('PVC') || q.type.includes('ฉาก')) pvcCount++; else if (q.type.includes('ม่านม้วน')) rollerCount++; else aluCount++; const qDate = new Date(q.date || q.id).toLocaleDateString('th-TH'); if (last7Days[qDate] !== undefined) last7Days[qDate] += amount; }); const recent = [...quotes].sort((a,b) => (b.id||0) - (a.id||0)).slice(0, 5); container.innerHTML = `<div class="max-w-5xl mx-auto space-y-6"><div class="flex justify-between items-center mb-2"><div><h3 class="text-2xl font-black text-slate-800">Overview</h3><p class="text-xs text-slate-400">ภาพรวมระบบล่าสุด</p></div><div class="text-right"><div class="text-xs font-bold text-slate-400">อัพเดทล่าสุด</div><div class="text-sm font-bold text-slate-600">${new Date().toLocaleTimeString('th-TH')}</div></div></div><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4"><div class="rounded-2xl p-5 bg-gradient-to-br from-pink-500 to-rose-600 text-white shadow-lg relative overflow-hidden group"><div class="absolute top-0 right-0 p-4 opacity-20 transform group-hover:scale-125 transition-transform"><svg class="w-16 h-16" fill="currentColor" viewBox="0 0 20 20"><path d="M2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"/><path d="M12 2.252A8.014 8.014 0 0117.748 8H12V2.252z"/></svg></div><div class="relative z-10"><div class="text-xs font-medium opacity-80 mb-1">ยอดเสนอราคา (ทั้งหมด)</div><div class="text-2xl font-black tracking-tight">${totalValue.toLocaleString(undefined, {maximumFractionDigits:0})} ฿</div><div class="mt-2 text-[10px] bg-white/20 inline-block px-2 py-0.5 rounded backdrop-blur-sm">System Total</div></div></div><div class="rounded-2xl p-5 bg-gradient-to-br from-violet-500 to-purple-600 text-white shadow-lg relative overflow-hidden group"><div class="absolute top-0 right-0 p-4 opacity-20 transform group-hover:scale-125 transition-transform"><svg class="w-16 h-16" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" /><path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3z" clip-rule="evenodd" /></svg></div><div class="relative z-10"><div class="text-xs font-medium opacity-80 mb-1">จำนวนใบเสนอราคา</div><div class="text-2xl font-black tracking-tight">${totalDocs} ใบ</div><div class="mt-2 text-[10px] bg-white/20 inline-block px-2 py-0.5 rounded backdrop-blur-sm">+${recent.length} Recent</div></div></div></div></div>`; } catch (e) { console.error("Dashboard Error:", e); container.innerHTML = `<div class="text-center text-red-400 py-10">เกิดข้อผิดพลาดในการโหลดข้อมูล: ${e.message}</div>`; } }

// --- PWA INSTALLATION & IOS SUPPORT (UPDATED) ---
let deferredPrompt;
...
function checkPwaStatus() { 
    ...
    // Always show if not installed (Hybrid Mode)
    ...
    if (isDeviceIOS) {
        ...
        const handleInstall = () => {
            if (deferredPrompt) {
                ...
                // If browser didn't fire event (e.g. debugging or not https), show guide
                document.getElementById('installGuideModal').classList.remove('hidden');
                const title = document.getElementById('installGuideTitle');
                if(title) title.innerText = "ติดตั้งแอพ";
                const instructions = document.getElementById('installInstructions');
                if(instructions) instructions.innerHTML = "กดปุ่ม <strong>'Install'</strong> หรือ <strong>'Add to Home Screen'</strong> ในเมนูของเบราว์เซอร์";
            }
        };
...
window.addEventListener('DOMContentLoaded', () => { 
    ...
    renderSidebar();
    setupAutocomplete();
    checkPwaStatus(); // Check button visibility on load
    if(typeof renderNews === 'function') renderNews();
