...
// [เพิ่มใหม่] ตัวแปรจำเป็นสำหรับระบบ Admin
let tempQuotes = []; 
let tempConfig = {}; 
let deferredPrompt; // สำหรับ PWA (Android)
...
function shareCurrentPage() {
    ...
    // ป้องกัน Error หาก calcMode ยังไม่มีค่า
    const mode = (typeof calcMode !== 'undefined') ? calcMode : 'HOME';
    const shareUrl = `${baseUrl}?mode=${mode}`;
...
function renderSidebar() {
    ...
    // ป้องกัน error หาก config ยังไม่โหลด
    ...
    menus.forEach(menu => {
        ...
        // Safe Access for ICONS
        let iconSvg = '';
        if(typeof ICONS !== 'undefined' && ICONS[menu.icon]) {
            iconSvg = ICONS[menu.icon];
        } else if (typeof ICONS !== 'undefined' && ICONS['wood']) {
            iconSvg = ICONS['wood'];
        } else {
            iconSvg = '📦'; // Fallback icon
        }
    ...
    if ((typeof appConfig !== 'undefined' && appConfig.calcSettings && appConfig.calcSettings.enabled) || isAdmin) {
        // ... (ส่วน Render ปุ่มคำนวณราคา เหมือนเดิม)
        ...
        // SVG Icons (Hardcoded for safety)
...
// --- [ADDED] ADMIN RENDERERS (ฟังก์ชันที่ขาดไป เพิ่มให้แล้ว) ---
...
function renderAdminCalcInputs() { 
    ...
    container.innerHTML = `
        ...
        <div class="space-y-6 pb-10">
            <div class="bg-amber-50 p-4 rounded-xl border border-amber-200"><h3 class="font-bold text-amber-800 border-b border-amber-200 pb-2 mb-3 flex items-center gap-2">🪵 มู่ลี่ไม้ (Wood)</h3><div class="grid grid-cols-2 gap-4"><div><label class="text-[10px] font-bold text-slate-500">ราคา Basswood</label><input type="number" value="${w.priceBasswood||0}" onchange="tempConfig.calcSettings.wood.priceBasswood = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-white"></div><div><label class="text-[10px] font-bold text-slate-500">ราคา Foamwood</label><input type="number" value="${w.priceFoamwood||0}" onchange="tempConfig.calcSettings.wood.priceFoamwood = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-white"></div><div><label class="text-[10px] font-bold text-slate-500">ตัวคูณ</label><input type="number" step="0.01" value="${w.factor||1}" onchange="tempConfig.calcSettings.wood.factor = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-white"></div></div></div>
            <div class="bg-blue-50 p-4 rounded-xl border border-blue-200"><h3 class="font-bold text-blue-800 border-b border-blue-200 pb-2 mb-3 flex items-center gap-2">🚪 ฉากกั้นห้อง (PVC)</h3><div class="grid grid-cols-2 gap-4"><div><label class="text-[10px] font-bold text-slate-500">ตัวคูณ</label><input type="number" step="0.01" value="${p.factor||1}" onchange="tempConfig.calcSettings.pvc.factor = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-white"></div><div><label class="text-[10px] font-bold text-slate-500">กว้างขั้นต่ำ</label><input type="number" step="0.01" value="${p.minW||0}" onchange="tempConfig.calcSettings.pvc.minW = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-white"></div></div></div>
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-200"><h3 class="font-bold text-slate-800 border-b border-slate-200 pb-2 mb-3 flex items-center gap-2">🪟 ม่านม้วน</h3><div class="grid grid-cols-2 gap-4 mt-2"><div><label class="text-[10px] font-bold text-slate-500">ตัวคูณผ้า</label><input type="number" step="0.1" value="${r.fabricMult||1}" onchange="tempConfig.calcSettings.roller.fabricMult = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-slate-50"></div><div><label class="text-[10px] font-bold text-slate-500">ค่าอุปกรณ์</label><input type="number" value="${r.eqExt||0}" onchange="tempConfig.calcSettings.roller.eqExt = parseFloat(this.value)" class="w-full p-2 border rounded text-sm bg-slate-50"></div></div></div>
        </div>
    `; 
}
...
function renderAdminMenu() {
    ...
    tempConfig.menus.forEach((menu, idx) => {
        // Safe Access for ICONS
        let iconDisplay = '?';
        if (typeof ICONS !== 'undefined' && ICONS[menu.icon]) iconDisplay = ICONS[menu.icon];
        ...
        list.innerHTML += `
            <div class="bg-white p-3 rounded-xl border border-slate-200 flex flex-col gap-3">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-slate-100 rounded text-slate-500">${iconDisplay}</div>
...
// Helpers for Menu (Attached to Window)
...
async function renderAdminDashboard() {
    ...
    try {
        ...
        quotes.forEach(q => {
            ...
            if (q.type && q.type.includes('ไม้')) woodCount++;
            else if (q.type && (q.type.includes('PVC') || q.type.includes('ฉาก'))) pvcCount++;
            else if (q.type && q.type.includes('ม่านม้วน')) rollerCount++;
            else aluCount++;
...
// --- PWA INSTALLATION (ANDROID ONLY - REMOVED IOS) ---
function checkPwaStatus() { 
    ...
    const handleInstall = () => {
        if (deferredPrompt) {
            ...
            // ถ้าไม่ใช่ Android หรือยังไม่พร้อม ให้แจ้งเตือนธรรมดาแทน Modal IOS
            alert("สามารถติดตั้งแอพได้จากเมนูของเบราว์เซอร์\nเลือก 'Install App' หรือ 'Add to Home Screen'");
        }
    };
                    ...
                    if (typeof configListenerSet === 'undefined' || !configListenerSet) {
                        db.collection("app_settings").doc("config").onSnapshot((doc) => {
                            if (doc.exists) {
                                ...
                                const adminModal = document.getElementById('adminConfigModal');
...
window.addEventListener('DOMContentLoaded', () => { 
    ...
    // 2. Render UI (Check existence first)
    if(typeof renderSidebar === 'function') renderSidebar();
    if(typeof setupAutocomplete === 'function') {
        setupAutocomplete();
    } else {
        console.warn("setupAutocomplete not found - skipping search setup");
    }
    ...
    // 4. *** FORCE REMOVE SPLASH SCREEN ***
    // (ทำงานแน่นอน 100% แม้มี error อื่น)
    setTimeout(() => {
