...
// [เพิ่มใหม่] ตัวแปรที่ขาดหายไป ต้องมีเพื่อให้ Admin Mode ทำงานได้
let tempQuotes = []; 
let tempConfig = {}; 
...
function shareCurrentPage() {
    ...
    // ป้องกัน error ถ้า calcMode ยังไม่ถูกประกาศ
...
function renderSidebar() {
    ...
    // ป้องกัน error กรณี config ยังไม่โหลด
...
// --- [ADDED] ADMIN RENDERERS (ฟังก์ชันที่ขาดหายไป) ---
// ส่วนนี้คือฟังก์ชัน Admin ที่สมบูรณ์จาก Code B นำมาใส่ใน Code A
...
// --- APP INIT (ใช้แบบเดิมของ Code A เพื่อความเสถียร ไม่ค้าง) ---
    ...
    try {
        ...
        auth.onAuthStateChanged((user) => {
            if (user) {
                ...
                // Config Listener (เพิ่มการเช็คตัวแปร configListenerSet)
                if (typeof configListenerSet === 'undefined' || !configListenerSet) {
                    db.collection("app_settings").doc("config").onSnapshot((doc) => {
                        if (doc.exists) {
                            ...
                            // ถ้าหน้า Admin เปิดอยู่ ให้รีโหลดข้อมูลด้วย
...
window.addEventListener('DOMContentLoaded', () => { 
    ...
    // [ป้องกันเว็บค้าง] เช็คว่าฟังก์ชันมีอยู่จริงไหมก่อนเรียก
    if(typeof setupAutocomplete === 'function') {
        setupAutocomplete(); 
    } else {
        console.warn("setupAutocomplete function is missing (skipped to prevent freeze)");
    }
    ...
    // ลบ Splash Screen เมื่อโหลดเสร็จ (ป้องกันค้างหน้าโลโก้)
